...
sample.archives <- list.files(root.dir, pattern="*.zip", full.names=TRUE)
## ME samples: MECFS001 - no CD8CD4, HD015v3 - broken data for CD8CD4
...
for (idx in seq_len(nrow(map))) {
    ...
    this.archives <- data.table(sample=this.sample, 
                                path=list.files(root.dir, pattern=this.pattern, full.names=TRUE))
...
for (idx in seq_len(nrow(map))) {
    ...
    this.pattern <- "*.gz"
    this.map <- data.table(sample=this.sample, 
...
unique.map <- unique(map, by="sample")
sample.map <- unique.map[nested.map, on="sample"]
...
## Run empty droplets and doublet decettion for all files
...
this.pattern <- "_All_"
subsample.map <- sample.map[!grepl(this.pattern, sample.dir), .(sample, sample.dir)]
metadata <- subsample.map[, unique(.SD), .SDcols=c("sample", "sample.dir")]
...
for (this.sample in sel.samples){
    ...
    subsample.map <- sample.map[grepl(this.pattern, sample.dir), .(sample, sample.dir)]
    single.subsample <- subsample.map[, unique(.SD), .SDcols=c("sample", "sample.dir")]
